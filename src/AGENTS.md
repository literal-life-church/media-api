...
- **`request.signal` abort listener** — cleans up the writer when the client disconnects, preventing memory leaks in the connections set. Note: the abort signal only fires on a clean TCP close. Clients that drop silently (e.g., closed tabs on poor connections) may leave stale writers in the set indefinitely.
- **Stale writer eviction** — never `await writer.write()` directly. Stale connections have a full, unconsumed readable-side queue; backpressure causes `write()` to block forever. Always use a two-stage guard: (1) check `writer.desiredSize === null || writer.desiredSize <= 0` synchronously and reject immediately, then (2) race the write against a short timeout. Use `Promise.allSettled()` to write to all connections concurrently so one stale writer cannot block others.
