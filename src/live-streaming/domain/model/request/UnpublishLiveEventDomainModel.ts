...
export const UnpublishLiveEventDomainModel = z.object({
...
}).describe("The data model for unpublishing a live event, which only includes the time the request was made.");

export function UnpublishLiveEventDomainModelSchema() {
    return {
        body: contentJson(UnpublishLiveEventDomainModel)
    };
}
